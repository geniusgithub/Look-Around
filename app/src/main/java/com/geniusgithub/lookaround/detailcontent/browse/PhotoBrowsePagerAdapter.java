...
class PhotoBrowsePagerAdapter extends PagerAdapter {
    ...
    private  void loadSource(Context context, String item, ViewHolder viewHolder){
        GlideImageView object = new GlideImageView(viewHolder.mPhoneView);
        object.bindViewHolder(viewHolder);
        Glide.with(context).load(item).crossFade().skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(object);
    }
